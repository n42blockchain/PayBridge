    ...
    <el-card shadow="never">
      <template #header>
        <div class="flex items-center justify-between">
          <span class="font-bold text-lg">充值渠道</span>
...
const columns: TableColumnList = [
  ...
  { label: "最小金额", prop: "minAmount", width: 120, slot: "minAmount" },
  { label: "最大金额", prop: "maxAmount", width: 120, slot: "maxAmount" },
  ...
  await dialogFormRef.value.validate(async valid => {
    if (valid) {
      ...
      try {
        ...
        if (isEdit.value) {
          await topupChannelApi.update(currentId.value, data as any);
          message("渠道更新成功", { type: "success" });
        } else {
          await topupChannelApi.create(data as any);
...
async function handleStatusChange(row: any, status: string) {
  try {
    await topupChannelApi.update(row.id, { status } as any);
...
async function handleDelete(row: any) {
  try {
    ...
    await topupChannelApi.update(row.id, { status: "DISABLED" } as any);
